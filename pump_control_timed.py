...
class PumpControlTimed:
    def __init__(self):
        ...
        #-----------------main parameters------------------------#
        self.run_time = 30.0 # seconds - how long the program should run
        self.interval = 10.0 # seconds - serial vals sent to arduino every interval seconds
        self.amount_dispensed = 0.2 #mL - amount of liquid to be dispensed
        #------------------------parameters----------------------#
        self.rate = 0.38 # rate in mL/s, this value does not change the speed of the pump
        self.to_arduino = 70.0 # serial val sent to arduino to turn on
        #-----------------calculated-values----------------------#
        self.pump_on = 0.0 # how long the pump stays on to pump an amount of liquid
        self.wait_time = 0.0 # how long the program has to wait before sending serial vals to arduino
        self.times_run = 0.0 # how many times the pump will turn on 
        
    
    # this method sends a numerical value (serial_val) to a serial port
    def write_to(self, serial_val):
        #print("sending ", serial_val, "to arduino")
        #converts the inputted value back to string
        serial_val = str(serial_val) 
        #sends value to arduino as byted encoded with utf-8
        self.arduino.write(bytes(serial_val, 'utf-8'))
        time.sleep(0.05)
    ...
    def set_vals (self, run_time, interval, amount_dispensed, rate):
      self.run_time = int(run_time) #in s
      self.interval = int(interval) #in s
      self.amount_dispensed = int(amount_dispensed) #in ul
      self.rate = (self.rate * 1000) #uL/s
      if self.rate != 0:  
          self.pump_on = self.amount_dispensed / self.rate  
      else:
          raise ValueError ("the rate cannot be 0")
      self.wait_time = self.interval - self.pump_on 
      self.times_run = int(run_time / interval)      
      
    def validate_input(self):
        # checks if the specified interval can be split evenly across the run time
        if(not self.run_time % self.interval == 0):
        ...
        # if(to_arduino is None or not 225 >= to_arduino >= 0):
    ...
    def print_time(self):
        ...
        #currentInterval = start_time
        end_time = start_time + timedelta(seconds = self.run_time)
        print("start: ", start_time)
        print("end: ", end_time)
        print(self.times_run, "samples will be taken over the time period")
    ...
    def run_last(self):
        print("sending ", self.to_arduino, "to arduino")
        self.write_to(self.to_arduino)
        time.sleep(0.5) #pause for aruduino to respond
        print("Read from Arduino:", self.read_from())
        time.sleep(self.pump_on) #time the pump is on 
       
    # Prints the time the program stopped running,
    # sends 0 to arduino to stop the pump, and
    # sets the value to_arduino to 0
    def end_run(self):
